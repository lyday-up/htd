...
// NewHtdGorm 代表实例化Gorm
func NewHtdGorm(params ...interface{}) (interface{}, error) {
...
// GetDB 获取DB实例
func (app *HtdGorm) GetDB(option ...contract.DBOption) (*gorm.DB, error) {
	logger := app.container.MustMake(contract.ZapLogKey).(contract.ZapLoger)

	// 读取默认配置
	config := GetBaseConfig(app.container)

	logService := app.container.MustMake(contract.ZapLogKey).(contract.ZapLoger)

	// 设置Logger
	ormLogger := NewOrmZapLogger(logService)
