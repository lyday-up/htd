...
import (
	...
	"github.com/htd/framework/provider/log/services"
)

// HtdLogServiceProvider 服务提供者
type HtdLogServiceProvider struct {
...
// Register 注册一个服务实例
func (l *HtdLogServiceProvider) Register(c framework.Container) framework.NewInstance {
	if l.Driver == "" {
		tcs, err := c.Make(contract.ConfigKey)
		if err != nil {
			// 默认使用console
			return services.NewHtdConsoleLog
		}
	...
	switch l.Driver {
	case "single":
		return services.NewHtdSingleLog
	case "rotate":
		return services.NewHtdRotateLog
	case "console":
		return services.NewHtdConsoleLog
	case "custom":
		return services.NewHtdCustomLog
	default:
		return services.NewHtdConsoleLog
	}
}
...
// Boot 启动的时候注入
func (l *HtdLogServiceProvider) Boot(c framework.Container) error {
	return nil
}

// IsDefer 是否延迟加载
func (l *HtdLogServiceProvider) IsDefer() bool {
	return false
}

// Params 定义要传递给实例化方法的参数
func (l *HtdLogServiceProvider) Params(c framework.Container) []interface{} {
	// 获取configService
...
// Name 定义对应的服务字符串凭证
func (l *HtdLogServiceProvider) Name() string {
